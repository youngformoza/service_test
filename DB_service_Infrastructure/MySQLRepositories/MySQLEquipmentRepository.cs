    ...
    public class MySQLEquipmentRepository : IBaseEquipmentRepository
    {
        ...
        public equipment GetByName(string name)
        {
            ...
            try
            {
                ...
                using var command = new MySqlCommand("SELECT id_eq, model, id_class, equipment_class.`name`, id_vendor, vendor.`name` FROM equipment " +
                    "JOIN equipment_class ON equipment.id_class = equipment_class.id_eq_cl " +
                    "JOIN vendor ON equipment.id_vendor = vendor.id_ven " +
                    "WHERE model = @name", connection);
        ...
        public equipment[] GetAllEquipment()
        {
            ...
            try
            {
                ...
                using var command = new MySqlCommand("SELECT id_eq, model, id_class, equipment_class.`name`, id_vendor, vendor.`name` FROM equipment " +
                "JOIN equipment_class ON equipment.id_class = equipment_class.id_eq_cl " +
                "JOIN vendor ON equipment.id_vendor = vendor.id_ven; ", connection);

                using var reader = command.ExecuteReader();

                while (reader.Read())
                {
                    equipmentList.Add(new equipment(reader.GetInt32(0), reader.GetString(1),
                        new equipment_class(reader.GetInt32(2), reader.GetString(3)),
                        new vendor(reader.GetInt32(4), reader.GetString(5))));
                }
            }
